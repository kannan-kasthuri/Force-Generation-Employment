...
# TOPIC MODELING PARAMETERS - Optimized for 3500 documents
NUM_TOPICS = None  # Set to None for automatic detection, or specify (e.g., 28)
MIN_TOPIC_SIZE = 10  # OPTIMIZED: ~1.4% of 3500 docs - aims for 20-30 topics
MAX_TOPICS = 35  # OPTIMIZED: Cap at reasonable number
UMAP_N_NEIGHBORS = 20  # OPTIMIZED: Larger for smoother manifold with 3500 docs
UMAP_N_COMPONENTS = 25  # OPTIMIZED: Balanced complexity
UMAP_MIN_DIST = 0.05  # OPTIMIZED: Not too tight to allow some flexibility
HDBSCAN_MIN_CLUSTER_SIZE = 10  # OPTIMIZED: Match min_topic_size
HDBSCAN_MIN_SAMPLES = 4  # OPTIMIZED: More conservative clustering
...
write_log(log_file, "Using HDBSCAN for clustering (better for topic discovery)...")
hdbscan_model = HDBSCAN(
    min_cluster_size=HDBSCAN_MIN_CLUSTER_SIZE,
    min_samples=HDBSCAN_MIN_SAMPLES,
    metric='euclidean',
    cluster_selection_method='eom',  # CHANGED: 'eom' is better for varying cluster sizes
...
write_log(log_file, f"Outlier documents (topic -1): {list(topics).count(-1)}")
...
if outlier_ratio > 0.15:
    recommendations.append(f"⚠ High outlier ratio ({outlier_ratio*100:.1f}%). Consider:")
    recommendations.append(f"  - Decreasing MIN_TOPIC_SIZE from {MIN_TOPIC_SIZE} to {max(5, int(MIN_TOPIC_SIZE * 0.7))}")
    recommendations.append("  - Using HDBSCAN cluster_selection_method='leaf' instead of 'eom'")
    recommendations.append("  - Check if documents are genuinely too diverse for topic modeling")
elif outlier_ratio > 0.05:
    recommendations.append(f"✓ Moderate outlier ratio ({outlier_ratio*100:.1f}%) - acceptable")
