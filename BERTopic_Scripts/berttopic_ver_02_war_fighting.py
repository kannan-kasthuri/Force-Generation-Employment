...
INPUT_FOLDER = f"{USAWC}\\Desktop\\Data\\USAWC_EDU\\Research\\SQRL\\SRR\\Results\\BERTopic\\extracted_warfighting_paragraphs_no_outliers"
OUTPUT_FOLDER = f"{USAWC}\\Desktop\\Data\\USAWC_EDU\\Research\\SQRL\\SRR\\Results\\BERTopic\\BERTopic_Results_No_Outliers_Warfighting_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

# TOPIC MODELING PARAMETERS - Optimized for paragraph-level modeling
NUM_TOPICS = None
MIN_TOPIC_SIZE = 23  # Increased to reduce fragmentation (ensures at least MIN_TOPIC_SIZE paragraphs would contain the identified topic)
MAX_TOPICS = 60     # Increased to allow more topics before aggressive reduction (affects the total number of topics idendified before reduction takes place)
UMAP_N_NEIGHBORS = 10  # Increased for broader structure
UMAP_N_COMPONENTS = 3  # Reduced for better reduction and speed
UMAP_MIN_DIST = 0.1  # Tighter clusters for paragraphs
HDBSCAN_MIN_CLUSTER_SIZE = 23  # Increased to match min_topic_size
HDBSCAN_MIN_SAMPLES = 5  # Increased to reduce noise sensitivity
