...
try:
    ...
    umap_model = cumlUMAP(
        ...
        metric='cosine',
        random_state=RANDOM_SEED  # Added seed
    )
...
original_num_outliers = num_outliers
original_outlier_percentage = outlier_percentage
...
# 1. Simple 2D Scatter Plot with documents colored by topic
try:
    ...
    # Get unique topics (excluding outliers for color mapping)
    ...
    # Plot outliers separately in gray
    if -1 in topics:
        mask = np.array(topics) == -1
        plt.scatter(embeddings_2d[mask, 0], embeddings_2d[mask, 1], 
                   c='lightgray', label='Outliers', 
    ...
    # Legend - only show if not too many topics
    # Replace this section (after the plotting loops, before plt.grid):

    # Add topic labels with arrows instead of legend
    try:
        # Calculate topic centroids
        topic_centroids = {}
        ...
        # Label only the N largest topics to avoid clutter
        N_TOPICS_TO_LABEL = min(30, num_topics)  # Adjust this number as needed
        
        # Get N largest topics by document count
        ...
        # Inside the loop for adding topic labels
        for topic_id in largest_topics:
            if topic_id in topic_centroids:
                ...
                # Manual adjustment for different topics
                if topic_id == 1:
                    xy = (0, 2.1)  # Force T1 to start at x=0, y=1.8
                    xytext = None  # No offset, place directly at xy
                elif topic_id == 2:
                    xy = (7.4, 9)
                    xytext = None
                elif topic_id == 3:
                ...
                # Add annotation
                plt.annotate(label, 
                            xy=xy,  # Use the manually set or calculated position
                            xytext=xytext,  # No offset unless specified
                            textcoords='data' if xytext is None else 'offset points',  # Use data coords for direct placement
                            fontsize=10,
                            ...
                            bbox=dict(boxstyle='round,pad=0.3', 
                                    ...
                                    edgecolor='black', 
                                    linewidth=0.5),
                            arrowprops=None) # dict(arrowstyle='->', connectionstyle='arc3,rad=0.3', color='black', lw=1.5, alpha=0.7)

        # After the annotation loop, set axes limits to ensure x=0 is visible
        plt.xlim(-2, 14.2)  # Match the approximate range from your image
        plt.ylim(-2, 10)    # Match the approximate range from your image
        
        # Add note about labeled topics
        # plt.text(0.02, 0.02, f'Showing top {N_TOPICS_TO_LABEL} largest topics (out of {num_topics} total)', 
        #        transform=plt.gca().transAxes, fontsize=9, 
        #        verticalalignment='bottom', 
        #        bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='gray'))
        
    except Exception as e:
        write_log(log_file, f"Could not add topic labels: {e}")
        # Fallback to simple legend if labeling fails
        if num_topics <= 20:
    ...
    # Also save the 2D embeddings for future use
    ...
    # 1b. Simple 2D Scatter Plot with original assignments (without outlier removal)
try:
    write_log(log_file, "Creating original document scatter plot (without outlier reassignment)...")
    ...
    # Get unique topics (excluding outliers for color mapping)
    unique_topics = sorted(set(original_topics) - {-1})
    colors = plt.cm.tab20(np.linspace(0, 1, len(unique_topics)))
    
    # Plot each topic
    for i, topic_id in enumerate(unique_topics):
        mask = np.array(original_topics) == topic_id
        plt.scatter(embeddings_2d[mask, 0], embeddings_2d[mask, 1], 
                   c=[colors[i]], label=f'Topic {topic_id}', 
                   alpha=0.6, s=50, edgecolors='white', linewidth=0.5)
    
    # Plot outliers separately as unclassified (hollow points with boundaries only)
    if -1 in original_topics:
        mask = np.array(original_topics) == -1
        plt.scatter(embeddings_2d[mask, 0], embeddings_2d[mask, 1], 
                   facecolors='none', edgecolors='black', label='Unclassified', 
                   alpha=0.5, s=30, linewidths=0.5)
    
    plt.xlabel('UMAP Dimension 1', fontsize=12)
    plt.ylabel('UMAP Dimension 2', fontsize=12)
    plt.title(f'Document Distribution by Topic (Original Assignments: n={len(documents)} docs, {original_num_topics} topics)', 
             fontsize=14, fontweight='bold')
    ...
    try:
        ...
        # Label only the N largest topics to avoid clutter
        N_TOPICS_TO_LABEL = min(30, original_num_topics)  # Adjust this number as needed
        
        # Get N largest topics by document count
        topic_counts_for_labels = pd.Series(original_topics).value_counts()
        largest_topics = topic_counts_for_labels[topic_counts_for_labels.index != -1].head(N_TOPICS_TO_LABEL).index.tolist()
        ...
        for topic_id in largest_topics:
            if topic_id in topic_centroids:
                ...
                # Manual adjustment for different topics
                if topic_id == 1:
                    xy = (0, 2)  # Force T1 to start at x=0, y=1.8
                    xytext = None  # No offset, place directly at xy
                elif topic_id == 2:
                    xy = (7.3, 9)
                    xytext = None
        ...
        # Add note about labeled topics
        # plt.text(0.02, 0.02, f'Showing top {N_TOPICS_TO_LABEL} largest topics (out of {original_num_topics} total)', 
        #       transform=plt.gca().transAxes, fontsize=9, 
        #       verticalalignment='bottom', 
        #       bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='gray'))
        
    except Exception as e:
        write_log(log_file, f"Could not add topic labels to original plot: {e}")
        # Fallback to simple legend if labeling fails
        if original_num_topics <= 20:
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', frameon=True, fontsize=9)
    ...
    # Save plot
    scatter_file_original = os.path.join(OUTPUT_FOLDER, "document_scatter_plot_original.png")
    plt.savefig(scatter_file_original, dpi=300, bbox_inches='tight')
    plt.close()
    write_log(log_file, f"Original document scatter plot saved to: {scatter_file_original}")
    
    # Also save the 2D embeddings for future use (already saved earlier, no need to resave)
    
except Exception as e:
    write_log(log_file, f"Could not create original scatter plot: {e}")
